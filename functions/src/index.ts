...
const checkRecentSentiment = (trend, getTweets, callback) => {
  ...
  twitter.getSearch({
    ...
    if (getTweets) {
      ...
      if (totalScore < 0) tweetList.sort((a, b) => a.sentiment - b.sentiment);
      if (totalScore > 0) tweetList.sort((a, b) => b.sentiment - a.sentiment);
    ...
    } else callback(totalScore.toFixed(2))
  });
};
